...
# Cursor 客户端配置 — AzaLoop 自动循环规则（V17）
...
### MCP 配置
- 使用 `.cursor/rules.md` 配置 MCP 服务器
- 确保 `aza_auto_loop` 工具已注册（见 `mcp.json`）
...
### 钩子脚本
- `hooks/pre-tool.js`：工具调用前执行
- `hooks/post-tool.js`：工具调用后执行，自动更新 STATE
- `hooks/on-stop.js`：停止时执行
