# AzaLoop V17 循环规则 — {{CLIENT_NAME}}

> 本文件由 `aza setup --client {{CLIENT_NAME}}` 从 `templates/_shared/v12.2-rules.md` 生成。
...
阶段无关工具（所有阶段可用）：`aza_auto_loop`、`aza_loop_*`、`aza_context_calibrate`、`aza_memory_*`、`aza_skill_*`、`aza_runstate_*`
...
- 规则文件路径：`{{RULES_PATH}}`
- 客户端能力 tier：{{CLIENT_TIER}}（T1 全自动 / T2 部分 / T3 基础——本客户端已升级为全自动）
